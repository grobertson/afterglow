...
# The if selection statement proceeds to the script if $1 is not empty.
   ...
   LOGFILE="$TMPFOLDER/plexpp-post-proc.log" # Create a unique log file.
   ...
   echo "$(date +"%Y%m%d-%H%M%S"): Starting transcode of $FILENAME to $TEMPFILENAME" 2>&1> $LOGFILE
   if [[ $ENCODER == "handbrake" ]]; then
     echo "You have selected HandBrake" 2>&1> $LOGFILE
     HandBrakeCLI -i "$FILENAME" -f mkv --aencoder copy -e qsv_h264 --x264-preset veryfast --x264-profile auto -q 16 --maxHeight $RES --decomb bob -o "$TEMPFILENAME" 2>&1> $LOGFILE
     check_errs $? "Failed to convert using Handbrake."
   elif [[ $ENCODER == "ffmpeg" ]]; then
     echo "You have selected FFMPEG" 2>&1> $LOGFILE
     ...
     echo "Oops, invalid ENCODER string.  Using Default [FFMpeg]." 2>&1> $LOGFILE
     ffmpeg -i "$FILENAME" -s hd$RES -c:v libx264 -preset veryfast -vf yadif -c:a copy "$TEMPFILENAME"
     check_errs $? "Failed to convert using FFMPEG."
   fi
   ...
   echo "$(date +"%Y%m%d-%H%M%S"): Finished transcode of $FILENAME to $TEMPFILENAME" 2>&1> $LOGFILE
   ...
   while [ true ] ; do
     if ls "$TMPFOLDER/"*".ppplock" 1> /dev/null 2>&1; then
       echo "$(date +"%Y%m%d-%H%M%S"): Looks like there is another scripting running.  Waiting." 2>&1> $LOGFILE
       sleep 5
     else
       echo "$(date +"%Y%m%d-%H%M%S"): It looks like all scripts are done running, exiting." 2>&1> $LOGFILE
       break
     fi
   done

   echo "$(date +"%Y%m%d-%H%M%S"): Encode done.  Exiting." 2>&1> $LOGFILE

else
   echo "********************************************************" 2>&1> $LOGFILE
   echo "PlexPostProc by nebhead" 2>&1> $LOGFILE
   echo "Usage: $0 FileName" 2>&1> $LOGFILE
   echo "********************************************************" 2>&1> $LOGFILE
fi
